...
from .llava_act_config import LlavaConfig
...
def select_with_start(tensor, start_tensor, k=100):
    batch_size, seq_len, hidden_size = tensor.shape
    ends = start_tensor
    starts = start_tensor - k

    cols = torch.arange(seq_len).to(tensor.device) 
...
def fill_chunk_input_mask(tensor, start_tensor, query_mask, k=2):
...
class LlavaCausalLMOutputWithPast(ModelOutput):
    ...
    # action chunk output states
    act_hidden_states: torch.FloatTensor = None
...
class LlavaForConditionalGeneration(LlavaPreTrainedModel):
    ...
    def _merge_input_ids_with_image_features(
        ...
        special_chunk_input_token_mask = input_ids == self.config.chunk_input_token_index
        # 计算batch中每个样本的图像数量
        num_special_image_tokens = torch.sum(special_image_token_mask, dim=-1)
        # Compute the maximum embed dimension
        # 加上chunk input size
        max_embed_dim = (num_special_image_tokens.max() * (num_image_patches - 1)) + sequence_length  + (self.config.chunk_size - 1)
        batch_indices, non_image_indices = torch.where(input_ids != self.config.image_token_index)
        # batch_indices_for_text, text_indices = torch.where((input_ids != self.config.image_token_index) & (input_ids != self.config.chunk_input_token_index))
        # 找到chunk input token 的位置，chunk input 占位符一定是在句子的结尾
        _, chunk_input_indices = torch.where(input_ids == self.config.chunk_input_token_index)
        # 2. Compute the positions where text should be written
        # Calculate new positions for text tokens in merged image-text sequence.
        # `special_image_token_mask` identifies image tokens. Each image token will be replaced by `nb_text_tokens_per_images - 1` text tokens.
        # `torch.cumsum` computes how each image token shifts subsequent text token positions.
        # - 1 to adjust for zero-based indexing, as `cumsum` inherently increases indices by one.
        new_token_positions = torch.cumsum((special_image_token_mask * (num_image_patches - 1) + 1) + special_chunk_input_token_mask * (self.config.chunk_size - 1), -1) - 1
        ...
        final_embedding[batch_indices_for_proprio, new_proprio_index] = proprio_input

        batch_indices_for_latent, latent_indices = torch.where(input_ids == self.config.latent_token_index)
        new_latent_index = new_token_positions[batch_indices_for_latent, latent_indices]
        final_embedding[batch_indices_for_latent, new_latent_index] = latent_input
        
        # 设置mask
        final_attention_mask[batch_indices, text_to_overwrite] = attention_mask[batch_indices, non_image_indices]
        # proprio和latent长度都为1，不用单独设置，chunk input需要单独设置
        chunk_mask = torch.ones(batch_size, self.config.chunk_size, dtype=torch.long, device=query_embeds.device)
        final_attention_mask = fill_chunk_input_mask(final_attention_mask, new_chunk_index, chunk_mask, k=self.config.chunk_size) 
        ...
        return final_embedding, final_attention_mask, position_ids, new_chunk_index
    ...
    def forward(
        ...
        logits = outputs[0]
        act_hidden_states = select_with_start(outputs["hidden_states"][-1], new_chunk_index, k=self.config.chunk_size)
        ...
        return LlavaCausalLMOutputWithPast(
            ...
            act_hidden_states=act_hidden_states
        )
